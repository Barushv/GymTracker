...
export function maxKg(weekObj){
  ...
  let m=null; weekObj.sub.forEach(sub=> sub.forEach(set=>{ const v=parseFloat(set.kg); if(isFinite(v)){ m=(m===null)?v:Math.max(m,v); }}));
  return (m===null)?'':String(m);
}
export function historyString(progress, person, day, ex){
  const node=ensurePath(progress,person,day,ex); return WKEYS.map(k=> maxKg(node[k]) || '').join(' | ');
}
function chipL(k,v){ return v && v!=='—' ? `<span class="chip">${k}: ${v}</span>` : ''; }
function parseCounts(s){ const re=/(\d+)[x×]/g; const arr=[]; let m; while((m=re.exec(s))!==null) arr.push(parseInt(m[1],10)); return arr.length?arr:[3]; }
function splitSubnames(name){ return name.split(' + ').map(s=>s.trim()); }
function setsFor(name,scheme){ const subs=splitSubnames(name); const counts=parseCounts(scheme); if(subs.length===1){ return [counts.reduce((a,b)=>a+b,0)]; } return subs.map((_,i)=> counts[i] || counts[0] || 3); }
