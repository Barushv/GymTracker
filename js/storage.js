...
export function loadAll(def){
  const st = LS.get(KEYS.state, def); if(!('notes' in st)) st.notes='';
  return { state:st, progress:LS.get(KEYS.progress, {}), oneRM:LS.get(KEYS.oneRM, {"fercho":{},"andy":{}}), theme:localStorage.getItem(KEYS.theme)||'emerald' };
}
